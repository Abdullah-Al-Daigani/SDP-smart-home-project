...
void callback(char *topic, byte *payload, unsigned int length) // the call back function which gets called with incoming MQTT messages
{
  ...
  int command = payload_JSON["command"];
...
void setup() // the setup function runs only once after startup
{
  ...
  while (!ina219.begin()) // wait until the power monitor object is active before continuing
    ;                     //
  while (!ina219_B.begin()) // wait until the power monitor object is active before continuing
    ;                     //
  ...
  IPAddress mqtt_server(192, WiFi.localIP()[1], WiFi.localIP()[2], 35);
...
void loop() // the loop function runs continuously
{
  ...
  if (now - lastMsg > 400)           // compare the current time with the time of the last message
  {                                  //
    ...
    PubJSON["power_A"] = ina219.getPower_mW();
    PubJSON["current_A"] = ina219.getCurrent_mA();
    PubJSON["voltage_A"] = ina219.getBusVoltage_V();

    PubJSON["power_in"] = ina219_B.getPower_mW();
    PubJSON["current_in"] = ina219_B.getCurrent_mA();
    PubJSON["voltage_in"] = ina219_B.getBusVoltage_V();
    ...
    client.publish("load_data", buffer, n); // sends the data to the host through the MQTT protocol
...
void reconnect() // reconnects to the MQTT client
{
  ...
  while (!client.connected()) // check whether the client is connected
  {
    ...
    {                                            //
      ...
      client.subscribe("command");            // resubscribe
    }
    else
    {
